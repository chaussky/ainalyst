...
class TechniqueCrossCheckTest(PrioritizationGovernanceBase):
    ...
    def test_a_technique_mismatch_is_flagged_with_both_values(self):
        ...
        self.assertIn("использует **MoSCoW**, а в 3.3 запланирована техника **WSJF**", result)
...
class ParticipantCrossCheckTest(PrioritizationGovernanceBase):
    ...
    def test_an_unplanned_scorer_is_flagged_and_the_scores_are_still_recorded(self):
        ...
        self.assertIn("нет среди участников, запланированных в 3.3", result)
        self.assertIn(BOTH, result)
        self.assertIn("✅ Оценки стейкхолдера", result)          # not a refusal
        self.assertIn("**Требований оценено:** 2", result)
    ...
    def test_a_planned_scorer_is_not_flagged_and_the_match_is_normalised(self):
        ...
        self.assertIn("✅ Оценки стейкхолдера", result)
        self.assertNotIn("нет среди участников, запланированных в 3.3", result)
...
class ReportReconcilesParticipationTest(PrioritizationGovernanceBase):
    ...
    def test_the_report_reconciles_participation(self):
        ...
        self.assertIn("## Запланированный подход (3.3)", report)
        self.assertIn("**Technique:** MoSCoW planned — MoSCoW used ✅", report)
        self.assertIn("**Criteria:** cost, risk", report)
        self.assertIn("1 из 2 запланированных участников", report)
        self.assertIn("Не оценивали: Head of Risk", report)
        self.assertIn("Оценивали, не будучи запланированными: Lead Architect", report)
    ...
    def test_criteria_alone_still_render_the_block(self):
        ...
        self.assertIn("## Запланированный подход (3.3)", report)
        self.assertIn("**Criteria:** cost, risk", report)
        self.assertIn("**Техника:** в 3.3 не запланирована", report)
    ...
    def test_a_bare_string_where_the_participants_belong_is_not_split_per_character(self):
        ...
        report = self._finalised_report()
        self.assertIn("## Запланированный подход (3.3)", report)          # it rendered
    ...
    def test_the_scored_ids_and_priorities_are_unaffected(self):
        ...
        self._plan_everything(technique="WSJF")
        report = self._finalised_report()
        self.assertIn("**Обновлено требований:** 2", report)
        self.assertIn("### 🔴 Must (1 шт.)", report)
        self.assertIn("### 🟠 Should (1 шт.)", report)
    ...
    def test_the_typo_warning_still_lands_in_the_header(self):
        ...
        header_end = lines.index("**Обновлено требований:** 0")
        warning = next(i for i, l in enumerate(lines) if "не соответствует ни один узел репозитория" in l)
...
class OnePersonTwoPlannedRolesTest(PrioritizationGovernanceBase):
    ...
    def test_the_report_says_when_one_person_covered_two_roles(self):
        ...
        report = save_prioritization_result(PROJECT, "S1")
        self.assertIn("2 из 2 запланированных участников", report)
        self.assertIn("1 стейкхолдер закрыл вместе 2 запланированных ролей", report)
...
class NoRegistryTest(BaseMCPTest):
    ...
    def test_the_report_does_not_claim_nobody_scored_when_it_cannot_tell(self):
        ...
        run_aggregation(PROJECT, "S1")
        report = save_prioritization_result(PROJECT, "S1")
        self.assertIn("## Запланированный подход (3.3)", report)      # the block rendered
