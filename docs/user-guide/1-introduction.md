...
**MCP-инструменты** — 22 сервера со 114 инструментами, которые выполняют конкретные аналитические операции: строят реестры стейкхолдеров, анализируют транскрипты интервью, создают матрицы трассировки, оценивают риски, формируют спецификации требований.
...
**Ещё кое-что происходит незаметно для вас.** На задаче, которая того стоит, первый слой делится на четырёх агентов: один ведёт диалог с вами, один разбивает задачу на шаги, а двое выполняют половины, намеренно разведённые друг от друга, — рассуждение над материалом и запись артефактов. По именам вы их не зовёте, и в том, как вы разговариваете с платформой, ничего не меняется. Смысл разделения практический: шаг, который пишет на диск, ничего не додумывает от себя, поэтому на одном и том же входе делает одно и то же, а неудавшийся шаг можно просто повторить. Заодно это означает, что агент, который читает ваш транскрипт, вообще не может ничего записать: он передаёт выводы дальше, а сохраняет их уже другой шаг.
...
Все результаты работы платформа сохраняет в папку **`governance_plans/reports/`** в формате Markdown. Это ваши рабочие артефакты: планы, реестры, спецификации, протоколы. Если нужен PDF — сконвертируйте `.md` любым привычным инструментом (например `pandoc` или «Печать в PDF» в Markdown-редакторе).
...
**Если один из этих файлов повредился.** Платформа хранит **последние пять версий** каждого внутреннего файла в `governance_plans/.history/` и заменяет файл в один приём — поэтому прерывание (Ctrl+C, кончившееся место на диске, севшая батарея ноутбука) оставляет прежнюю версию целой, а не наполовину записанной. Если инструмент сообщил, что файл не читается, — в его сообщении названы папка и файл; скопируйте поверх него самую свежую подходящую версию из `.history/`. Одна оговорка, и сообщение говорит её тоже: свежая копия — это проект **в том виде, в каком он был ДО последнего изменения**, поэтому именно последнее изменение она вернуть не может. Скажите AIналитику — он выполнит восстановление за вас и назовёт, что удалось вернуть.
