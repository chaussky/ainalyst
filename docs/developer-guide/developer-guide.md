...
The top layer can be split into four agents, defined in `.claude/agents/`. This is not a fourth
level between the others — it is the same top layer, divided by responsibility:

- **`analyst`** — the dialogue-driven BABOK work described in `CLAUDE.md`. The leading session
  plays this role by default; the file exists so a task can also run in an isolated context.
- **`orchestrator`** — decomposes a task, routes each step, serializes writes to shared artifacts.
- **`worker_llm`** — reasons over content and returns structured JSON. No file system at all.
- **`worker_file`** — reads files and calls the chapters' MCP tools.

The split is optional: a task that is one reasoning step plus one tool call goes straight to the
analyst. It pays off when a task splits into several steps, when independent steps can run in
parallel, or when several steps write into the same artifact and must be serialized.

The boundaries are enforced by the `tools:` grant in each agent's frontmatter, not by the prose in
its body. `tools:` is a strict allowlist that also excludes MCP tools, so `worker_llm` with
`tools: Read` physically cannot write an artifact, and `analyst`/`worker_file` are granted the MCP
servers by pattern (`mcp__babok-ch3`, …) rather than by listing 114 tool names. The protocol is in
`.claude/rules/agent_orchestration.md`.
...
| `.claude/` | Claude Code hooks, rules, agents, and settings |
| `.claude/agents/*.md` | The four agents: `analyst`, `orchestrator`, `worker_llm`, `worker_file` |
...
├── agents/
│   ├── analyst.md       ← dialogue-driven BABOK work, chapter → tool map
│   ├── orchestrator.md  ← decomposition, routing, serialization of writes
│   ├── worker_llm.md    ← reasoning over content, no file system
│   └── worker_file.md   ← files and the chapters' MCP tools
└── rules/
    ├── artifacts.md              ← rules for working with artifacts
    ├── babok_process.md          ← rules for following the BABOK process
    └── agent_orchestration.md    ← the protocol for the four agents
```
...
**`agents/`** holds the four agents. Each file is flat Markdown: YAML frontmatter (`name`, `description`, `tools`) plus the body, which becomes the agent's system prompt. There is no build step and no shared includes — Claude Code reads these files as they are.

Three properties of the harness matter when editing them. A subagent inherits every `CLAUDE.md` and every file in `rules/`, so an agent file must not restate them; it does not inherit the conversation, so everything a worker needs goes into the prompt that launches it. A subagent shares the leading session's phase and has no `.mcp.json` of its own. And **a changed agent file only takes effect after a session restart** — verify edits in a fresh session, not the current one.

**`rules/`** holds behavior rules that Claude Code follows while it works. `artifacts.md` describes how to name and save artifacts; `babok_process.md` covers the rules for following the BABOK methodology in the dialogue with the BA; `agent_orchestration.md` describes how the four agents divide the work, what each one's tool grant enforces, and what they return to each other.
