...
Верхний слой можно развернуть в четырёх агентов, описанных в `.claude/agents/`. Это не
четвёртый уровень между остальными — это тот же верхний слой, разделённый по зонам
ответственности:

- **`analyst`** — диалоговая работа по BABOK, описанная в `CLAUDE.md`. Ведущая сессия играет
  эту роль по умолчанию; файл нужен, чтобы задачу можно было выполнить и в отдельном контексте.
- **`orchestrator`** — раскладывает задачу на шаги, маршрутизирует каждый, сериализует записи
  в общие артефакты.
- **`worker_llm`** — рассуждает над контентом и возвращает structured JSON. Файловой системы нет
  вовсе.
- **`worker_file`** — читает файлы и вызывает MCP-инструменты глав.

Разделение опционально: задача из одного рассуждения и одного вызова инструмента идёт прямо к
аналитику. Оно окупается, когда задача распадается на несколько шагов, когда независимые шаги
можно вести параллельно или когда несколько шагов пишут в один артефакт и требуют сериализации.

Границы держит грант `tools:` во frontmatter каждого агента, а не проза в его теле. `tools:` —
строгий белый список, отрезающий и MCP-инструменты, поэтому `worker_llm` с `tools: Read`
физически не может записать артефакт, а `analyst` и `worker_file` получают MCP-серверы шаблоном
(`mcp__babok-ch3`, …), а не перечислением 114 имён. Протокол — в
...
| `.claude/` | Хуки, правила, агенты и настройки Claude Code |
| `.claude/agents/*.md` | Четыре агента: `analyst`, `orchestrator`, `worker_llm`, `worker_file` |
...
├── agents/
│   ├── analyst.md       ← диалоговая работа по BABOK, карта «глава → инструмент»
│   ├── orchestrator.md  ← декомпозиция, маршрутизация, сериализация записей
│   ├── worker_llm.md    ← рассуждение над контентом, без файловой системы
│   └── worker_file.md   ← файлы и MCP-инструменты глав
└── rules/
    ├── artifacts.md              ← правила работы с артефактами
    ├── babok_process.md          ← правила соблюдения BABOK-процесса
    └── agent_orchestration.md    ← протокол работы четырёх агентов
```
...
**`agents/`** — четыре агента. Каждый файл — плоский Markdown: YAML-frontmatter (`name`, `description`, `tools`) плюс тело, которое становится системным промптом агента. Шага сборки и общих включений нет — Claude Code читает эти файлы как есть.

При правке важны три свойства харнесса. Субагент наследует все `CLAUDE.md` и все файлы из `rules/`, поэтому файл агента не должен их пересказывать; разговора он не наследует, поэтому всё, что нужно воркеру, кладётся в промпт запуска. Субагент живёт в фазе ведущей сессии, своего `.mcp.json` у него нет. И **изменённый файл агента вступает в силу только после перезапуска сессии** — проверять правки нужно в свежей сессии, а не в текущей.

**`rules/`** — правила поведения Claude Code, которые он учитывает при работе. `artifacts.md` описывает как именовать и сохранять артефакты; `babok_process.md` — правила следования BABOK-методологии в диалоге с BA; `agent_orchestration.md` — как четыре агента делят работу, что закрепляет грант инструментов каждого и что они возвращают друг другу.
