...
[![Tests](https://github.com/chaussky/ainalyst/actions/workflows/tests.yml/badge.svg?branch=ru)](https://github.com/chaussky/ainalyst/actions/workflows/tests.yml)
...
Ошибки и предложения — через GitHub Issues; по остальному — `chaussky@gmail.com`. Анонсы обновлений и обсуждение с другими пользователями — в Телеграм-канале **[t.me/platform_ainalyst](https://t.me/platform_ainalyst)**.
...
На больших задачах этот верхний уровень может разделиться на четырёх агентов: один ведёт диалог с вами, один разбивает задачу на шаги, а двое выполняют половины, которые нельзя смешивать, — рассуждение над материалом и запись артефактов. По именам вы их не зовёте, и в вашей работе ничего не меняется: платформа прибегает к разделению, когда задача того стоит. Смысл разделения практический: шаг, который пишет на диск, остаётся детерминированным, поэтому неудавшийся шаг можно просто повторить.
...
AI Платформа AIналитик (AI-powered Platform AInalyst) распространяется под лицензией **GNU AGPL v3**. AGPL разрешает многое, в том числе коммерческое использование, при одном условии: тот, кто получил программу — включая пользователя, который работает с ней только по сети, — получает и полный исходный код под той же лицензией.

**Одной AGPL достаточно для:**
...
- Любого, кто дорабатывает платформу, размещает её у себя или строит на ней своё решение и готов открыть свой исходный код под AGPL

**Коммерческая лицензия освобождает от этого условия.** Она нужна, чтобы:
- Предоставлять AIналитик как размещённый сервис (SaaS), не открывая свой исходный код
- Встроить код в коммерческий продукт с закрытым исходным кодом
- Выпустить платный графический интерфейс, не открывая код интерфейса
- Получить гарантии, возмещение убытков или выделенную поддержку — AGPL прямо снимает их с автора
